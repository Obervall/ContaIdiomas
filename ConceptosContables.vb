...
Public Class ConceptosContables
    ...
    Private Sub ConceptosContables_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        ' Llenar Grid de CONCEPTOS e Idioma
    ...
    Private Sub CargarYTraducirGridCompleto()
        ...
        'TraducirContenidoGridConceptos(DgvConceptos, rmse)
    End Sub
    ...
    Private Sub EjecutarBusquedaConceptos(ByVal forzarDesdeInicio As Boolean)
        ...
        Dim buscarTexto As String = vBuscar.ToLower().Trim()
        ...
        For i As Integer = filaInicio To DgvConceptos.Rows.Count - 1
            ...
            If FilaCumpleCondicionBusqueda(fila, vCampo, buscarTexto, exacta) Then
                vRow = i
    ...
    Private Function FilaCumpleCondicionBusqueda(ByVal fila As DataGridViewRow, ByVal indiceCampo As Integer, ByVal textoBuscar As String, ByVal esExacta As Boolean) As Boolean
        Dim celdasAEvaluar As New List(Of Integer)()
        If indiceCampo = 0 Then
            For c As Integer = 0 To fila.Cells.Count - 1
                celdasAEvaluar.Add(c)
            Next
        Else
            celdasAEvaluar.Add(indiceCampo - 1)
        End If
        ...
        For Each idx As Integer In celdasAEvaluar
            If idx < fila.Cells.Count AndAlso fila.Cells(idx).Value IsNot Nothing Then
                ...
                If esExacta Then
                    If contenidoCelda = textoBuscar Then Return True
                Else
                    If contenidoCelda.Contains(textoBuscar) Then Return True
                End If
            End If
        Next
        Return False
    ...
    'Public Sub TraducirContenidoGridConceptos(ByVal grid As DataGridView, ByVal manejadorRecursos As System.Resources.ResourceManager)
    '    Try
    ...
    '                    Dim conceptoOriginal As String = If(fila.Cells(1).Value?.ToString().Trim(), "")
    '                    Dim notaOriginal As String = If(fila.Cells(3).Value?.ToString().Trim(), "")

    '                    Dim llaveConcepto As String = conceptoOriginal.Replace(" ", "_")

    '                    ' --- COLUMNA (0): Tipo de Concepto ---
    '                    Dim tradTipo As String = manejadorRecursos.GetString("Tipo_" & tipoOriginal.Replace(" ", "_"))
    '                    If Not String.IsNullOrEmpty(tradTipo) Then fila.Cells(0).Value = tradTipo

    '                    ' --- COLUMNA (1): Nombre del Concepto ---
    '                    Dim tradConcepto As String = manejadorRecursos.GetString(llaveConcepto)
    '                    If Not String.IsNullOrEmpty(tradConcepto) Then fila.Cells(1).Value = tradConcepto

    '                    ' --- COLUMNA (3): Notas del Concepto (Solo "ESPECIAL") ---
    '                    If tipoOriginal.ToUpper() = "ESPECIAL" Then
    '                        Dim llaveNota As String = "Nota_" & llaveConcepto
    '                        Dim tradNota As String = manejadorRecursos.GetString(llaveNota)

    '                        If Not String.IsNullOrEmpty(tradNota) Then
    '                            fila.Cells(3).Value = tradNota
    '                        Else
    '                            fila.Cells(3).Value = notaOriginal
    '                        End If
    '                    End If
    ...
    '        ' Control de errores silencioso para el Grid
    '    End Try
    'End Sub
    ...
    '    ''' <summary>
    '    ''' Consulta la BD para volcar los registros en el Grid y activa la traducción híbrida
    '    ''' </summary>
    '    Private Sub CargarYTraducirGridCompleto()
    '        vtipoSql = "SELECT conceptos.TipoCON, conceptos.CodigoCON, conceptos.DescripcionCON, conceptos.NotasCON FROM conceptos"
    '        vtipoSql += " ORDER BY conceptos.CodigoCON ASC"
    ...
    '    ''' <summary>
    '    ''' Procesa el filtrado en base al valor real de la BD y traduce los resultados en pantalla
    '    ''' </summary>
    '    Private Sub FiltrarYTraducirGrid()
    '        Dim tipoParaDB As String = ""
    '        Select Case CmbTipoConcepto.SelectedIndex
    ...
    '    ' --- EL MOTOR UNIFICADO DE BÚSQUEDA COMPLETO ---
    '    Private Sub EjecutarBusquedaConceptos(ByVal forzarDesdeInicio As Boolean)
    '        vBuscar = frmBuscar.CmbTextoBuscar.Text
    '        vCampo = frmBuscar.CmbCampos.SelectedIndex
    ...
    '            ' Evaluamos si la fila actual cumple las condiciones de coincidencia
    '            If FilaCumpleCondicionBusqueda(fila, vCampo, buscarTexto, exacta) Then
    '                vRow = i
    '                DgvConceptos.CurrentCell = DgvConceptos.Rows(i).Cells(0)
    '                coincidenciaEncontrada = True
    ...
    '        ' Si no encontramos nada y no empezamos desde cero, avisamos al usuario
    '        If Not coincidenciaEncontrada Then
    '            MsgBox(resManager.GetString("NoSeEncontraronCoincidencias"), vbInformation, resManager.GetString("ToolTipBuscar"))
    ...
    '    ''' <summary>
    '    ''' Valida si el contenido de una celda coincide con el texto buscado
    '    ''' </summary>
    '    Private Function FilaCumpleCondicionBusqueda(ByVal fila As DataGridViewRow, ByVal indiceCampo As Integer, ByVal textoBuscar As String, ByVal esExacta As Boolean) As Boolean
    '        ' Si es 0 busca en todos los campos, de lo contrario evalúa la celda específica (indiceCampo - 1)
    '        Dim celdasAEvaluar As New List(Of Integer)()
    '        If indiceCampo = 0 Then
    '            For c As Integer = 0 To fila.Cells.Count - 1
    '                celdasAEvaluar.Add(c)
    ...
    '    ''' <summary>
    '    ''' Motor híbrido que traduce Celdas del Grid (Tipos, Códigos y Notas de elementos ESPECIAL)
    '    ''' </summary>
    '    Public Sub TraducirContenidoGridConceptos(ByVal grid As DataGridView, ByVal manejadorRecursos As System.Resources.ResourceManager)
    '        Try
    '            If grid IsNot Nothing AndAlso grid.Rows.Count > 0 Then
    ...
    '                        ' Obtención segura de valores crudos (pueden venir DBNull de la BD)
    '                        Dim tipoOriginal As String = If(fila.Cells(0).Value?.ToString().Trim(), "")
    '                        Dim conceptoOriginal As String = If(fila.Cells(1).Value?.ToString().Trim(), "")
    '                        Dim notaOriginal As String = If(fila.Cells(3).Value?.ToString().Trim(), "")
    '                        Dim llaveConcepto As String = conceptoOriginal.Replace(" ", "_")
    ...
    '                        Dim tradConcepto As String = manejadorRecursos.GetString(llaveConcepto)
    '                        If Not String.IsNullOrEmpty(tradConcepto) Then fila.Cells(1).Value = tradConcepto
    '                        ' --- COLUMNA (3): Notas del Concepto (Exclusivo para el Tipo "ESPECIAL") ---
    '                        If tipoOriginal.ToUpper() = "ESPECIAL" Then
    '                            Dim llaveNota As String = "Nota_" & llaveConcepto
    '                            Dim tradNota As String = manejadorRecursos.GetString(llaveNota)
    '                            If Not String.IsNullOrEmpty(tradNota) Then
    ...
    '                Next
    '            End If
    '        Catch ex As Exception ' Control de errores silencioso para evitar interrupciones visuales
    '        End Try
    '    End Sub
    ...
    Private Sub DgvConceptos_CellFormatting(sender As Object, e As DataGridViewCellFormattingEventArgs) Handles DgvConceptos.CellFormatting
        ' 1. Filtro de seguridad: Evitar filas vacías, cabeceras o la fila de inserción nueva
        If e.RowIndex < 0 OrElse DgvConceptos.Rows(e.RowIndex).IsNewRow Then Exit Sub

        Try
            ' 2. Validación crítica: Nos aseguramos de que las celdas de datos existan y no estén vacías en este milisegundo
            If DgvConceptos.Rows(e.RowIndex).Cells(0).Value Is Nothing OrElse
           DgvConceptos.Rows(e.RowIndex).Cells(1).Value Is Nothing Then
                Exit Sub
            End If

            ' --- TRADUCIR COLUMNA (0): TipoCON ---
            If e.ColumnIndex = 0 Then
                Dim valorOriginalBD As String = DgvConceptos.Rows(e.RowIndex).Cells(0).Value.ToString().Trim().ToUpper()
                Dim traduccion As String = ""

                Select Case valorOriginalBD
                    Case "GASTO" : traduccion = rmse.GetString("Tipo_Gasto")
                    Case "INGRESO" : traduccion = rmse.GetString("Tipo_Ingreso")
                    Case "ESPECIAL" : traduccion = rmse.GetString("Tipo_Especial")
                End Select
            ...
            If e.ColumnIndex = 1 Then
                Dim conceptoOriginal As String = DgvConceptos.Rows(e.RowIndex).Cells(1).Value.ToString().Trim()
                Dim llaveConcepto As String = conceptoOriginal.Replace(" ", "_")

                Dim tradConcepto As String = rmse.GetString(llaveConcepto)
                If Not String.IsNullOrEmpty(tradConcepto) Then
                    e.Value = tradConcepto
                    e.FormattingApplied = True
                End If
            End If

            ' --- TRADUCIR COLUMNA (3): NotasCON (SOLO SI EL TIPO ES "ESPECIAL") ---
            If e.ColumnIndex = 3 Then
                ' Leemos el tipo real de la celda (0) de esta misma fila
                Dim tipoFila As String = DgvConceptos.Rows(e.RowIndex).Cells(0).Value.ToString().Trim().ToUpper()

                ' Filtro estricto: Solo si en la BD es de tipo "ESPECIAL"
                If tipoFila = "ESPECIAL" Then
                    ' Tomamos el nombre original del concepto de la celda (1) para armar la clave de la nota
                    Dim conceptoOriginal As String = DgvConceptos.Rows(e.RowIndex).Cells(1).Value.ToString().Trim()
                    Dim llaveNota As String = "Nota_" & conceptoOriginal.Replace(" ", "_")

                    ' Buscamos la nota en el recurso global (resManager)
                    Dim tradNota As String = resManager.GetString(llaveNota)

                    ' Si no está en el global, la buscamos en el local del formulario (rmse)
                    If String.IsNullOrEmpty(tradNota) Then
                        tradNota = rmse.GetString(llaveNota)
                    End If

                    ' Si encontramos la traducción en cualquiera de los dos lados, la aplicamos
                    If Not String.IsNullOrEmpty(tradNota) Then
                        e.Value = tradNota
                        e.FormattingApplied = True
                    End If
        ...
        Catch ex As Exception
            ' Captura cualquier inicialización a medias sin interrumpir el programa
        End Try
    End Sub
